...
`POST /api/register` → `UserCreateRequest` → `UserController@register` → `UserRepository::create` → guarded OTP.
...
- `POST /api/token` — already supported `where('email',…)->orWhere('phone_number',…)`. Phone-only user `phone+password` → `data.token`.
...
- `GET /api/me` — Marvel `UserResource` returns `{ …, email: null, email_verified_at: null, phone_number, … }`. No exception.
...
- Cart ownership is `user_id` (`carts.user_id`, `cart_items.cart_id`). No email use.
  - **PASS** `phone_only_user_can_add_and_read_cart` (POST `/api/cart` 201 + GET `/api/cart` 200). Label: VERIFIED BY AUTOMATED TEST
...
- Views already guard: `resources/views/pdf/invoice.blade.php: {{ $customer['email'] ?? '' }}`, `resources/views/emails/order/order-invoice.blade.php: @if(isset($customer['email']))`, `resources/views/pdf/order-invoice.blade.php: @if(isset(...))`. Null renders as empty.
...
**Real HTTP:** `Real gateway HTTP call not executed` — `MyfatoraService` is mocked via `Mockery` (`createInvoice` returns `InvoiceURL/InvoiceId`); the unit-level resolver + mocked gateway together prove the boundary is correct without hitting MyFatoorah's API. Label: `NOT EXECUTED` (external)
...
- `Marvel\Http\Resources\UserResource` (`/api/me`, `/register`, `/token`) — serializes `email_verified_at` (null when no email). `apiResponse` wraps under `data`.
...
- [x] targeted tests green (CustomerWithoutEmail 13/13, PhoneOnlyE2ET 15/15, SocialLogin 17/17, Authentication 45/45, Invoice 34/34)
...
*Verified by code + automated tests. Report file: `new/email_vreifction.md`*
